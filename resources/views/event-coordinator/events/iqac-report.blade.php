...
<div class="container-fluid">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('event-coordinator.events.show', $event->id) }}" class="btn btn-outline-secondary btn-sm">Back to Event</a>
  </div>
  ...
  <div class="card border-0 shadow-sm mb-3">
    <div class="card-body">
      <div class="row g-2">
        ...
        <div class="col-md-4"><strong>Venue:</strong> {{ $event->venue ?: '-' }}</div>
        <div class="col-md-4"><strong>Dates:</strong> {{ optional($event->start_date)->format('d M Y') }} - {{ optional($event->end_date)->format('d M Y') }}</div>
      </div>
    </div>
  </div>
  ...
  <div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body">
      <form method="POST" action="{{ route('event-coordinator.events.iqac-reports.store', $event->id) }}" enctype="multipart/form-data">
        @csrf
        <div class="row g-3">
          <div class="col-md-4">
            <label class="form-label">Report Title</label>
            <input type="text" name="report_title" class="form-control" value="{{ old('report_title') }}" maxlength="255" placeholder="e.g. Annual Cultural Event Completion Report">
          </div>
          <div class="col-md-3">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="d-flex gap-2">
                <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editEcIqacReportModal{{ $report->id }}">Edit</button>
                <form method="POST" action="{{ route('event-coordinator.events.iqac-reports.destroy', [$event->id, $report->id]) }}" onsubmit="return confirm('Delete this IQAC report?');">
                  @csrf
                  @method('DELETE')
            ...
            <div class="modal fade" id="editEcIqacReportModal{{ $report->id }}" tabindex="-1" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <form method="POST" action="{{ route('event-coordinator.events.iqac-reports.update', [$event->id, $report->id]) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
            ...
            @empty
            <tr>
              <td colspan="7" class="text-center text-muted">No IQAC reports submitted yet.</td>
            </tr>
            @endforelse
          </tbody>
        </table>
