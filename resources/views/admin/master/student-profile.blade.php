...
@php
$activeSemesterId = (int) ($data->activeSemesterConfig?->semester_id ?? 0);
@endphp
...
<div class="sp-content">
  ...
  <div class="sp-panel active" id="tab-about">
      ...
      <div class="sp-info-grid">
        <div class="sp-info-item">
          ...
          <div>
            ...
            @endphp
            <div class="val">{{ $dobTs ? date('d M Y', $dobTs) : '—' }}</div>
          </div>
        </div>
        <div class="sp-info-item">
    ...
    <div class="sp-card">
      ...
      <div class="sp-info-grid">
        ...
        <div class="sp-info-item">
          ...
          <div>
            ...
            @php
            $profileSemesterId = (int) ($activeSemesterId ?: ((int) ($examStudent->current_semester ?? 0)));
            $profileSemesterTitle = optional(($availableSemesters ?? collect())->firstWhere('id', $profileSemesterId))->title;
            @endphp
            <div class="val">{{ $profileSemesterTitle ?: ($profileSemesterId > 0 ? 'Semester ' . $profileSemesterId : '—') }}</div>
          </div>
        </div>
        <div class="sp-info-item">
      ...
      @if(in_array($userRole, ['super-admin','admin','itcell','central-office']))
      <div class="mt-3" style="border-top:1px dashed #d8dee9;padding-top:1rem;">
        <div class="mb-2">
          <span class="badge bg-info text-dark">
            Current Active Semester:
            {{ optional(($availableSemesters ?? collect())->firstWhere('id', $activeSemesterId))->title ?? ('Semester ' . ($activeSemesterId ?: '-')) }}
          </span>
        </div>
        @if($activeSemesterId <= 0)
          <div class="alert alert-warning py-2 px-3 mb-2">
          No active semester configured yet in student_semester_configs. Please select a semester and update.
      </div>
      @endif
      <form method="POST" action="{{ route('admin.student.semester.update', ['id' => $data->id]) }}" class="row g-2 align-items-end">
        @csrf
        @method('PUT')
        <div class="col-md-7">
          <label class="form-label fw-semibold mb-1">Manually Update Student Semester</label>
          <select name="semester_id" class="form-select" required>
            <option value="">Select semester</option>
            @foreach(($availableSemesters ?? collect()) as $semester)
            @php
            $isSelected = $activeSemesterId === (int) $semester->id;
            @endphp
            <option value="{{ $semester->id }}" {{ $isSelected ? 'selected' : '' }}>{{ $semester->title }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="col-12">
          <small class="text-muted">This updates active semester in student_semester_configs and syncs Exam Student current semester.</small>
        </div>
      </form>
    </div>
    @endif
  ...
  <form method="POST" action="{{ route('admin.student.update', $data->id) }}">
    ...
    <div class="sp-card">
      ...
      <div class="row g-3">
        ...
        <div class="col-md-3">
          ...
          @php
          $dobInputTs = !empty($data->dob) ? strtotime((string) $data->dob) : false;
          $dobInputValue = $dobInputTs ? date('Y-m-d', $dobInputTs) : '';
          @endphp
          <input type="date" name="dob" class="form-control form-control-sm @error('dob') is-invalid @enderror"
            value="{{ old('dob', $dobInputValue) }}">
    ...
    <div class="sp-card">
      ...
      <div class="row g-3">
        ...
        @if(in_array($userRole, ['super-admin','admin','itcell','central-office']))
        <div class="col-md-6">
          <label class="form-label fw-semibold" style="font-size:.82rem;">Manual Semester Update</label>
          <select name="semester_id" class="form-select form-select-sm @error('semester_id') is-invalid @enderror">
            <option value="">Keep current semester</option>
            @foreach(($availableSemesters ?? collect()) as $semester)
            @php
            $isSelected = (int) old('semester_id', $activeSemesterId) === (int) $semester->id;
            @endphp
            <option value="{{ $semester->id }}" {{ $isSelected ? 'selected' : '' }}>{{ $semester->title }}</option>
            @endforeach
          </select>
          @error('semester_id')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
          <small class="text-muted">Current: {{ optional(($availableSemesters ?? collect())->firstWhere('id', $activeSemesterId))->title ?? ('Semester ' . ($activeSemesterId ?: '-')) }}</small>
          @if($activeSemesterId <= 0)
            <div class="small text-warning mt-1">No active semester row exists yet. Saving this form with a semester will create it.
        </div>
        @endif
      </div>
      @endif
