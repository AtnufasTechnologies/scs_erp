  ...
  <div class="row">
    ...
    <div class="modal fade" id="shiftProgram{{ $item->id }}" tabindex="-1" aria-labelledby="shiftProgram{{ $item->id }}" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="shiftProgram{{ $item->id }}">Program Transfer - {{ $item->applicationinfo->application_code }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form action="{{ route('admission.ug.phase1.shift-program', $item->id) }}" method="POST">
              ...
              <div class="mb-3">
                ...
                <label for="">Currect Program</label>
                ...
                <select class="form-select dselect-example" id="new_program{{ $item->id }}" name="new_program" required>
                  ...
                  <option value="{{ $program->id }}">{{ $program->studentprograminfo->code ?? '-'}} - {{ $program->studentprograminfo->name ?? '-'}} Available ({{ $program->total_available_seats }})</option>
                  @endforeach
                </select>
              </div>
              <div class="modal-footer">
