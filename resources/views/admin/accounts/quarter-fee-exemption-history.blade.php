...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h3 class="mb-0"><span class="text-uppercase">Quarterly Fee Exemption History</span></h3>
...
<div class="card mb-3">
  <div class="card-body">
    <form method="GET" action="{{ route('quarter.fee.exemptions.history') }}" class="row g-2 align-items-end">
      <div class="col-lg-3">
      ...
      <div class="col-lg-2">
        ...
        <select name="fee_structure_id" class="form-select">
          ...
          <option value="{{ $fs->id }}" {{ (string) request('fee_structure_id') === (string) $fs->id ? 'selected' : '' }}>{{ $fs->quarter_title }} (Y{{ $fs->std_current_year }})</option>
          @endforeach
        </select>
      </div>
      <div class="col-lg-2">
      ...
      <div class="col-lg-2 d-grid">
        <a href="{{ route('quarter.fee.exemptions.history') }}" class="btn btn-light border">Reset</a>
      </div>
    </form>
  </div>
</div>
      ...
      <table class="table table-bordered table-hover mb-0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $row->feeStructure->quarter_title ?? 'N/A' }} @if($row->feeStructure)<span class="text-muted">(Y{{ $row->feeStructure->std_current_year }})</span>@endif</td>
            <td>{{ $row->reason }}</td>
