...
class StudentRosterEngine
{
  ...
  private function resolveNormalRoster(int $courseId, ProgramCourseMaster $course, array $context): Collection
  {
    ...
    $resolved = $students->filter(function ($student) use ($course, $context, $comboById, $comboMapByProgramId, $curriculumRows, $studentSpecs) {
      ...
      $matchingRows = $curriculumRows->filter(function ($row) use ($eligibleComboIds, $semesterId, $student, $studentSpecs) {
        ...
        if (!$this->passesSpecializationGate($row, (int) ($student->student_id ?? 0), $studentSpecs)) {
          return false;
        }
      ...
      $isDual = $this->isDualPathway($student, $combo1, $combo2);
      ...
      foreach ($matchingRows as $row) {
        ...
        if ($deliveryType === ProgramWiseSemesterCourse::DELIVERY_MAJOR_COMBO1) {
          if ($selectedCombo > 0 && $combo1 > 0 && $selectedCombo !== $combo1) {
            continue;
          }
          return true;
        }
        ...
        if ($deliveryType === ProgramWiseSemesterCourse::DELIVERY_MAJOR_COMBO2) {
          if ($selectedCombo > 0 && $combo2 > 0 && $selectedCombo !== $combo2) {
            continue;
          }
          return true;
        }

        if ($deliveryType === ProgramWiseSemesterCourse::DELIVERY_OPEN_CHOICE && $selectionType === ProgramWiseSemesterCourse::TYPE_STUDENT_CHOICE) {
          if ($isDual) {
            $majorA = $combo1;
            $majorB = $combo2;
            ...
            if (($majorA > 0 && $offeringDept === $majorA) || ($majorB > 0 && $offeringDept === $majorB)) {
              continue;
            }
          }
      ...
      return false;
    })->map(function ($student) use ($courseId, $context) {
      return [
